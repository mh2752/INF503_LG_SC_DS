...
FASTAreadset_Chain::FASTAreadset_Chain()
{
    ...
    // Allocate memories for the
    // hash table. Exit gracefully
    // upon allocation failure:
    try
    {
        hash_table = new LinkedList*[hash_table_size];

        // Initialize all the cells to NULL:
...
// Overloaded Ctor():
// Default Ctor()
FASTAreadset_Chain::FASTAreadset_Chain(unsigned long int table_size)
{
    ...
    {
        cout<<"\n\t\tException Caught in Overloaded Constructor While Trying to Allocate Memory For the Hash Table. Exiting Program Now."<<endl;
        cout<<"\n\t\t\tException Details: "<<e.what()<<endl;
        exit(EXIT_FAILURE);
    }
...
FASTAreadset_Chain::~FASTAreadset_Chain()
{
    ...
    if(hash_table!=NULL)
    {
        // Call the destructor of each linked list
        // stored in the hash table explicitely:
        for(unsigned long int index = 0; index < hash_table_size; index++)
        {   
           if(hash_table[index]!=NULL)
           {
                // Call LinkedList's Dtor():
...
unsigned long int FASTAreadset_Chain::Get_Radix_Digit_Value(char radix_digit)
{
...
unsigned long int FASTAreadset_Chain::Get_Radix_Of_The_Sequence(const char* sequence)
{
...
void FASTAreadset_Chain::Clear_Buffer_Array(char* &given_buffer)
{
...
void FASTAreadset_Chain::Clear_Buffer_Array_With_Length(char* &given_buffer,unsigned int length)
{
...
void FASTAreadset_Chain::Read_In_Read_Data_Set(const char* filepath)
{
        ...
        else
        {
            ...
            {                
                ...
                while(!stream_to_data_file.eof())
                {
                    ...
                    else // It's a sequence. Read it and insert into the hash table
                    {
                        ...
                        // If the seq_radix > (hash_table_size -1), insert the
                        // sequence into the LinkedList at the last index and
                        // increment the total_collisons:
                        if(seq_radix > (hash_table_size-1))
                        {
                            // Set seq radix to the last index:
                            seq_radix = hash_table_size-1;

                            // If the index doesn't have
                            // a LinkedList, create one
                            // and insert this sequence:
                            if(hash_table[seq_radix] == NULL)
                            {
                                // Not a collision. Just create the LinkedList
                                // and insert the item into the list:
                                LinkedList *new_list;
                                new_list = new LinkedList;
                                new_list->Insert_Item_At_Front(buffer,max_sequence_length);

                                // Now, add this LinkedList into
                                // the hash table:
                                hash_table[seq_radix] = new_list;
                            }
                            else
                            {
                                // Insert the item:
                                hash_table[seq_radix]->Insert_Item_At_Front(buffer,max_sequence_length);
                        ...
                        else // seq_radix within range. Go ahead.
                        {
                            // If the hash table doesn't have
                            // a LinkedList at this index, create one
                            // and insert this sequence:
                            if(hash_table[seq_radix] == NULL)
                            {
                                // Not a collision. Just create the LinkedList
...
void FASTAreadset_Chain::Read_In_Bacillus_Genome(const char* filepath)
{
...
void FASTAreadset_Chain::Generate_Bacillus_16mers()
{
...
bool FASTAreadset_Chain::Search_Hash_Table_For_Given_16mer(const char* given_16mer)
{
   ...
   try
   {
       ...
       // If the radix_value > hash_table_size-1,
       // reset it to hash_table_size-1
       if(radix_value > (hash_table_size-1))
       {
           radix_value = hash_table_size-1;
       }

        // Now, search the hash table:
        if(hash_table[radix_value]==NULL)
        {
            return false;
        }
        else
        {
            // We assume the item is not
            // present by default. 
            bool search_result = false;

            // Search the LinkedList at this
            // index in hash_table for the
            // given 16mer sequence:
            hash_table[radix_value]->Search_Item(given_16mer,length_of_a_16mer,search_result);
...
unsigned long int FASTAreadset_Chain::Get_Hash_Table_Size()
{
...
unsigned long int FASTAreadset_Chain::Get_Total_Number_Of_Hash_Collisions()
{
...
unsigned long int FASTAreadset_Chain::Get_Num_Of_Unique_Hash_Keys()
{
...
unsigned long int FASTAreadset_Chain::Get_Total_Number_Of_16mers()
{
...
unsigned long int FASTAreadset_Chain::Search_Hash_Table_For_16mers(unsigned long int search_limit)
{
    ...
    {  
        ...
        // Setting up search limit if parameter value is 0 or greater
        // than the number of 16mers generated:

        if(search_limit < 1 || search_limit > num_of_16mers)
        {
            search_limit = num_of_16mers;
        }


        for(unsigned long int i = 0; i < search_limit; i++)
        {
